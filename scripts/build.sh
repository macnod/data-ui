...
function usage {
    ...
    echo 
    echo "This script builds the docker containers for ${APPLICATION} and"
...
VALUES_FILE="charts/${APPLICATION}/values-${ENVIRONMENT}.yaml"
...
helm template $RELEASE_NAME ./charts/${APPLICATION} \
    --namespace misc \
    --values "$VALUES_FILE" \
...
if [[ "$NO_DEPLOY" = false ]]; then
    helm upgrade -install $RELEASE_NAME ./charts/${APPLICATION} \
        --namespace misc \
        --create-namespace \
        --values "$VALUES_FILE" \
        --set image.tag=$VERSION
