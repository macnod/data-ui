...
(defun compile-model (model)
  ":private: Compiles code provided with :CHECKS-FN and :SOURCE-FN field
attributes. This makes running the code faster and safer, and it also allows the
system to raise an error early on if the code fails to compile."
  (if (u:plistp model)
    (loop for key in model by #'cddr
      for value in (cdr model) by #'cddr
      for compiled = (if (member key '(:checks-fn :source-fn))
                       (compile nil `(lambda (it) ,value))
                       (compile-model value))
      collect key
      collect compiled)
    model))
...
(defun validate-resource-types (&optional (types *resource-types*))
  ...
  (loop for type-key in (u:plist-keys types)
    for type-attributes = (u:plist-keys (getf types type-key))
    for fields = (u:tree-get types type-key :fields)
    unless type-attributes
    do (error "Resource type ~a is not defined." type-key)
    unless (u:has *valid-type-keys* type-attributes)
    do (error "Invalid resource type keys in type ~(~a~): ~{~(~a~)~^, ~}."
         type-key
	       (set-difference type-attributes *valid-type-keys* :test 'eql))
    unless (u:plistp fields)
    do (error ":FIELDS value is not a plist: ~a" fields)
    do (loop for field-key in fields by #'cddr
         do (validate-field type-key field-key types))))
...
(defun validate-field (type-key field-key &optional (types *resource-types*))
  (if (u:tree-get types type-key field-key :reference)
    (list :reference (validate-reference-field type-key field-key types))
    (list :regular (validate-regular-field type-key field-key types))))

(defun validate-regular-field (type-key field-key &optional (types *resource-types*))
  (let* ((field (u:tree-get types type-key field-key))
          (unknown-keys (set-difference (u:plist-keys field) *valid-field-keys*
                         :test 'eql))
          (type (getf field :type :text))
          (input-type (getf field :input-type :line))
          (default (getf field :default))
          (required (getf field :required))
          (unique (getf field :unique))
          (checks-fn (getf field :checks-fn))
          (options-fn (getf field :options-fn))
          (errors nil))
    (unless field (error "Field ~a lacks a specification." field-key))
    (when unknown-keys
      (push
        (format nil
          "~@(~r~) unknown field attribute~:p in field ~(~a~):~(~a~): ~{~a~^, ~}."
              (length unknown-keys) type-key field-key unknown-keys)
        errors))
